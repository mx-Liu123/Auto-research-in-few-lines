...
def inner_product_from_fft(a_fft_data, b_fft_data, PSD, dt, original_length, fmin=None, fmax=None, use_gpu=False, debug=False, maximize_phase=False, maximize_time=False):
    """
    ...
    Args:
        ...
        maximize_phase (bool, optional): Whether to maximize over phase. Default is False
        maximize_time (bool, optional): Whether to maximize over time. Default is False
    ...
    if use_gpu:
        freq_np = freq.get()  # Convert to numpy for comparison
    else:
        freq_np = freq
    
    if fmin is not None or fmax is not None:
        if fmin is not None:
            mask_min = (freq_np[1:] > fmin)  # Exclude DC component
        if fmax is not None:
            mask_max = (freq_np[1:] < fmax)
        ...
        freq_mask = xp.full(len(freq_np) - 1, True, dtype=bool)  # -1 to exclude DC
    ...
    # Apply frequency mask and compute inner product integrand
    if is_complex:
        ...
        integrand = 4 * ((a_fft_plus_masked.conj() * b_fft_plus_masked + 
        ...
        integrand = 4 * ((a_fft_masked.conj() * b_fft_masked) / 
                        PSD[:, freq_mask])
    
    # Sum over channels
    integrand_sum = integrand.sum(axis=0)

    if maximize_time:
        # Put masked integrand back into full frequency array
        full_integrand = xp.zeros(original_length // 2 + 1, dtype=xp.complex128)
        # freq_mask corresponds to frequencies from index 1
        full_integrand[1:][freq_mask] = integrand_sum
        
        if maximize_phase:
            # Analytic signal for phase maximization: Magnitude of complex correlation
            padded = xp.zeros(original_length, dtype=xp.complex128)
            padded[:len(full_integrand)] = full_integrand
            # Correct scaling for ifft with 4* integrand: (1.0 / dt)
            z_t = xp.fft.ifft(padded) * (1.0 / dt)
            inner_prod = xp.max(xp.abs(z_t))
        else:
            # Traditional time maximization: Real part of correlation
            # Correct scaling for irfft with 4* integrand: (0.5 / dt)
            z_t_real = xp.fft.irfft(full_integrand, n=original_length) * (0.5 / dt)
            inner_prod = xp.max(z_t_real)
    else:
        # Standard calculation at t=0
        complex_inner_prod = (integrand_sum * df).sum()
        if maximize_phase:
...
def calculate_overlap_1pa_vs_2pa(m1_1pa, m2_1pa, a_1pa, p0_1pa, e0_1pa, Y0_1pa, 
                               ...
                               waveform_2pa_fft=None, xp=None, use_gpu=False, maximize_phase=False, maximize_time=False):
    ...
    if N_1pa < N_fiducial:
        ...
        waveform_1pa = [
            xp.pad(xp.asarray(waveform_1pa[0]), (0, pad_length), mode='constant', constant_values=0),
            xp.pad(xp.asarray(waveform_1pa[1]), (0, pad_length), mode='constant', constant_values=0),
            xp.pad(xp.asarray(waveform_1pa[2]), (0, pad_length), mode='constant', constant_values=0)
        ]
    elif N_1pa > N_fiducial:
        waveform_1pa = [
            waveform_1pa[0][:N_fiducial],
            waveform_1pa[1][:N_fiducial],
            waveform_1pa[2][:N_fiducial]
        ]
    ...
    h1_dot_h2 = inner_product_from_fft(waveform_2pa_fft, waveform_1pa_fft, PSD, dt, N_fiducial, use_gpu=use_gpu, maximize_phase=maximize_phase, maximize_time=maximize_time)
    # Normalization terms:
    h1_dot_h1 = inner_product_from_fft(waveform_1pa_fft, waveform_1pa_fft, PSD, dt, N_fiducial, use_gpu=use_gpu, maximize_phase=False, maximize_time=False)
    h2_dot_h2 = TARGET_SNR**2
    
    # Overlap calculation
    overlap = np.abs(h1_dot_h2) / np.sqrt(max(h1_dot_h1 * h2_dot_h2, 1e-30))
...
def calculate_optimal_snr_1pa_vs_2pa(m1_1pa, m2_1pa, a_1pa, p0_1pa, e0_1pa, Y0_1pa, 
    ...
    h1_dot_h2 = inner_product_from_fft(waveform_2pa_fft, waveform_1pa_fft, PSD, dt, N_fiducial, use_gpu=use_gpu, maximize_phase=maximize_phase, maximize_time=maximize_time)
    h1_dot_h1 = inner_product_from_fft(waveform_1pa_fft, waveform_1pa_fft, PSD, dt, N_fiducial, use_gpu=use_gpu, maximize_phase=False, maximize_time=False)
    h2_dot_h2 = inner_product_from_fft(waveform_2pa_fft, waveform_2pa_fft, PSD, dt, N_fiducial, use_gpu=use_gpu, maximize_phase=False, maximize_time=False)

    # Overlap calculation (using standard definition)
